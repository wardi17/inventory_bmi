...
<div id="main">
    ...
    event.preventDefault();
     ...
     let data =validasiinput(event);
      ...
      if(data !==false){
        ...
        $("#CreateAdd").fadeOut();
        $.ajax({
                url:"<?=base_url?>/InvTransferfg/updatedata",
                method:"POST",
                dataType: "json",
                data:data,
  ...
  function  GetkodeTransada(userid,SoTransacID){
    $("#loadingModal").show();
      $.ajax({
                  url:"<?=base_url?>/invtransferfg/Tampildata",
...
function getListItem(){
  ...
  function edit_row(transno,itemno,warehouse,warehouse2,batchno,Refno,PartId,Quantity,keterangan){
  $("#edit_row").fadeOut();
  $("#delete").fadeOut();
  $("#warehouse").val(warehouse);
  $("#warehouse2").val(warehouse2);
  $("#refNo").val(Refno);
  $("#BatchNo").val(batchno);
  $("#pcs").val(Quantity);
